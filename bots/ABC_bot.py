...
def generate_signals(data, granularity, p1=None, p2=None, threshold=None):
    """
    Generates trading signals based on the given hyperparameters.

    Parameters:
        data (pd.DataFrame): The dataset containing price data.
        p1 (int, optional): The window size for the short-term moving average.
        p2 (int, optional): The window size for the long-term moving average.
        threshold (float, optional): The threshold for generating buy/sell signals.

    Returns:
        np.ndarray: An array of trading signals.
    """
    if p1 is None or p2 is None or threshold is None:
        p1, p2, threshold = abc_optimizer()

    # Adjust behavior based on granularity
    if granularity == "minute":
        # Use smaller window sizes for minute data
        p1 = max(1, p1 // 10)
        p2 = max(5, p2 // 10)
        threshold /= 10
    elif granularity == "hourly":
        # Use moderately smaller window sizes for hourly data
        p1 = max(2, p1 // 2)
        p2 = max(10, p2 // 2)
        threshold /= 2
